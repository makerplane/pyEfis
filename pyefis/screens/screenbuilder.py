...
class Screen(QWidget):
    ...
    def init_screen(self):
        ...
        if len(self.encoder_list) > 0:
            print(f"self.encoder:{self.encoder}, self.encoder_button:{self.encoder_button}")
            if self.encoder and self.encoder_button:
                ...
                print("Setup dbkeys")
            print(f"sorted encoder list: {self.encoder_list_sorted}")
            ...
            for option,value in i['options'].items():
                if 'encoder_order' == option:
                    if callable(getattr(self.instruments[count], 'enc_selectable', None)):
                        print(f"Instrument[{count}] is selectable and has order: {i['options']['encoder_order']}")
    ...
    def encoderChanged(self,value=0):
        ...
        # Whenever non zero value log current timestamp
        # Whenever zero value check if logged timestamp is is older than timeout, if so then call timeout operation.
        ...
        print(f"Unselect {self.encoder_current_selection}, select {val}, isEnabled:{self.instruments[self.encoder_list_sorted[val]].isEnabled()}")
        #print(dir(self.instruments[self.encoder_list_sorted[val]]))
        loop = 0
        if not self.instruments[self.encoder_list_sorted[val]].isEnabled():
            print(f"Button {val} is disabled")
